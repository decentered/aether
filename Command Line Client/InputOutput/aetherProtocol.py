...
class AetherProtocolFactory(ClientFactory):
    ...
    def overseer(self):
        # Reassign print to its original meaning in this context.
        def print(*a, **kwargs):
           return __builtin__.print(*a, **kwargs)
        date_format='%m/%d/%Y %H:%M:%S %Z'
        date = datetime.now(tz=pytz.utc)
        date = date.astimezone(pytz.timezone('US/Pacific'))
        print(len(self.openConnections), 'connections open at', date.strftime(date_format))
        arr = []
        for conn in self.openConnections:
            tempString = conn.connectionState
            arr.append(tempString)
        print('The current statuses of these connections are:', arr)
